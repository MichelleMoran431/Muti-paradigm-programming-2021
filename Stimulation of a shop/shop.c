...
void printProduct(struct Product p) // void means nothing is returned from function 
{
	printf("PRODUCT NAME: %s \nPRODUCT PRICE: %.2f\n", p.name, p.price); //  displays statement in quotes
	printf("-------------\n");
}
...
struct Shop createAndStockShop()
{
	...
	return shop; // exiting from the funtion shop
}
...
struct Customer customerorder()
{
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
		...
		struct Product product = { name, price };
		struct ProductStock stockItem = { product, quantity };
		struct ProductStock shoppingList[10] = { product, quantity };
		shop.stock[shop.index++] = stockItem;
		// printf("NAME OF PRODUCT %s PRICE %.2f QUANTITY %d\n", name, price, quantity);
    }
...
int main(void) // start of the function main() 
{
